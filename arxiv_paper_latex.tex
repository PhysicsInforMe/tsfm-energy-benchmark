...
\author{
  ...
  \texttt{luigi.simeone@techmanagement.net}
}
...
Time Series Foundation Models (TSFMs) offer a distinct alternative. Pre-trained on large corpora of heterogeneous time series data, these models generate forecasts for previously unseen series without any task-specific fitting---a capability referred to as zero-shot forecasting. Recent architectures include Amazon's Chronos family \citep{ansari2024chronos}, which tokenises continuous values and applies encoder-decoder transformers; Salesforce's Moirai \citep{woo2024unified}, a universal transformer trained on the LOTSA dataset exceeding 27 billion observations; IBM's TinyTimeMixer (TTM) \citep{ekambaram2024tiny}, an MLP-Mixer architecture with fewer than one million parameters; Google's TimesFM \citep{das2024decoder}, a decoder-only model; and Lag-Llama \citep{rasul2023lag}, which adapts the LLaMA architecture for probabilistic forecasting.
...
\textit{Chronos-Bolt} \citep{ansari2024chronos} uses a T5-based encoder-decoder transformer. Time series values are tokenised through scaling and quantisation, and the model is trained with a cross-entropy loss over the token vocabulary. We use the \textit{small} variant ($\sim$48M parameters).

\textit{Chronos-2} \citep{ansari2024chronos} employs an encoder-only architecture with grouped query attention (GQA). We use the \textit{small} variant ($\sim$46M parameters). GQA reduces the key-value head count, improving inference efficiency while maintaining representational capacity.

\textit{Moirai-2} \citep{woo2024unified} is a universal transformer with any-variate attention, pre-trained on the LOTSA dataset (27B+ observations from multiple domains). We use the \textit{small} variant ($\sim$14M parameters). Moirai processes raw continuous values rather than tokens.

\textit{TinyTimeMixer (TTM)} \citep{ekambaram2024tiny} replaces the transformer with an MLP-Mixer operating on patched time series segments. With fewer than 1M parameters, it is over an order of magnitude smaller than the other foundation models and is designed for scenarios where computational resources are limited.
...
\begin{tabular}{lcccc}
\toprule
Period & Chronos-Bolt & Moirai-2 & TTM & Prophet \\
\midrule
Normal Summer (ref.) & 0.342 & 0.378 & 0.456 & 0.653 \\
COVID Lockdown       & 0.308 ($-$10\%) & 0.319 ($-$16\%) & 0.452 ($-$1\%) & 0.657 (+1\%) \\
Recent 2023          & 0.405 (+18\%) & 0.395 (+4\%) & 0.700 (+54\%) & 0.649 ($-$1\%) \\
Winter Storm Uri     & 0.930 (+172\%) & \textbf{0.861} (+128\%) & 1.216 (+167\%) & 1.498 (+129\%) \\
Holiday Period       & 1.076 (+215\%) & \textbf{0.894} (+137\%) & 1.487 (+226\%) & 1.224 (+87\%) \\
\bottomrule
\end{tabular}
\end{table}
...
Two patterns emerge. Under mild distributional shift (COVID lockdowns), all models either maintain or improve accuracy. The COVID period reduced demand variability as commercial loads became more predictable, and this regularity benefited the foundation models. Under severe shift (Winter Storm Uri, holidays), all models degrade substantially, but the magnitude varies. Moirai-2 consistently shows the lowest degradation: 128\% during Uri versus 172\% for Chronos-Bolt and 167\% for TTM. This robustness advantage likely reflects Moirai's training on the LOTSA dataset, which includes time series from domains with extreme events (healthcare, finance), giving the model exposure to distributional tails during pre-training.
...
\caption{Left: MASE during extreme high-load and extreme low-load events. All models degrade relative to normal conditions, but Moirai-2 maintains the lowest error for extreme highs (0.21). Right: 90\% prediction interval coverage during extreme events. Prophet and TTM maintain near-target coverage; Chronos-Bolt and Moirai-2 drop below the 90\% target, indicating that their intervals are calibrated for normal conditions but not for distributional tails.}
\label{fig:extreme_events}
\end{figure}

During extreme high-load events, Moirai-2 achieves the lowest MASE (0.21), followed by Chronos-Bolt (0.22). Prophet and TTM show substantially higher errors (0.29 and 0.70, respectively). For extreme low-load events, the ranking shifts: Moirai-2 remains strongest (0.27) while Chronos-Bolt degrades more (0.45). The calibration panel reveals an important asymmetry: Prophet and TTM maintain near-target coverage during extremes (at the cost of wide intervals), while Chronos-Bolt and Moirai-2 drop below the 90\% target, suggesting their uncertainty estimates are calibrated for the body of the demand distribution rather than its tails.
...
\caption{Energy storage optimisation under uncertainty. Top: spot price trajectory. Middle: optimal charge/discharge schedule. Bottom: battery state of charge with net benefit. The optimiser charges during low-price overnight hours and discharges during the morning price peak; the negative net benefit in this period reflects insufficient price spread to offset cycling costs.}
\label{fig:storage}
\end{figure}
...
\begin{itemize}
    ...
    \item \textbf{Robustness:} Exhibits the lowest coefficient of variation (0.268) in MASE across summer, winter, and COVID test periods, indicating stable performance under varying demand regimes.
    \item \textbf{Latency:} At 31\,ms per 24-hour forecast, inference speed is adequate for real-time operational deployment. Moirai-2 is faster (26\,ms), but the 5\,ms difference is negligible for day-ahead scheduling applications.
\end{itemize}
...
\textit{For robustness under distribution shift:} Moirai-2, but with mandatory post-hoc calibration. Its pre-training on the diverse LOTSA dataset translates to the lowest degradation under extreme events (+128\% during Uri versus +172\% for Chronos-Bolt), but its 71\% empirical coverage requires correction before operational use.
...
\textit{Smallest model variants.} We use the small variants of Chronos-Bolt ($\sim$48M parameters), Chronos-2 ($\sim$46M), and Moirai-2 ($\sim$14M). Larger variants with more parameters may achieve better accuracy and calibration, though at increased computational cost that may exceed consumer hardware capabilities.
...
ERCOT data are publicly available via the EIA Open Data API (\url{https://api.eia.gov}). The complete benchmark framework---including data retrieval scripts, model wrappers, evaluation pipelines, raw results for all 2{,}352 forecast scenarios, and figure reproduction code---is available at: \url{https://github.com/luigisimeone/tsfm-energy-benchmark}. All experiments are fully reproducible on consumer hardware (AMD Ryzen~7 or equivalent, 16\,GB RAM, no GPU required). Model versions and Python package pinning are specified in the repository's \texttt{requirements.txt}.
